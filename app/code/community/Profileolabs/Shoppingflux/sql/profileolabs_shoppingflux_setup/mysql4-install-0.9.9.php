...
try {
    $enterpriseOrderGrid = $this->getTable('enterprise_salesarchive/order_grid');
} catch (Exception $e) {
    $enterpriseOrderGrid = false;
}

try {
    $enterpriseInvoiceGrid = $this->getTable('enterprise_salesarchive/invoice_grid');
} catch (Exception $e) {
    $enterpriseInvoiceGrid = false;
}

try {
    $enterpriseCreditmemoGrid = $this->getTable('enterprise_salesarchive/creditmemo_grid');
} catch (Exception $e) {
    $enterpriseCreditmemoGrid = false;
}
...
if (!$attribute = $salesInstaller->getAttribute($entityTypeId, 'from_shoppingflux')) {
    ...
    try {
        if ($enterpriseOrderGrid) {
            $salesInstaller->run("ALTER TABLE `{$enterpriseOrderGrid}` ADD `from_shoppingflux` INTEGER(11) NULL;");
        }
    } catch (Exception $e) {
    }
}
...
if (!$salesInstaller->getAttribute($entityTypeId, 'order_id_shoppingflux')) {
    ...
    try {
        if ($enterpriseOrderGrid) {
            $salesInstaller->run("ALTER TABLE `{$enterpriseOrderGrid}` ADD `order_id_shoppingflux` VARCHAR(255) NULL;");
        }
    } catch (Exception $e) {
    }
}
...
if (!$attribute = $salesInstaller->getAttribute($entityTypeId, 'marketplace_shoppingflux')) {
    ...
    try {
        if ($enterpriseOrderGrid) {
            $salesInstaller->run("ALTER TABLE `{$enterpriseOrderGrid}` ADD `marketplace_shoppingflux` VARCHAR(255) NULL;");
        }
    } catch (Exception $e) {
    }
}
...
if (!$attribute = $salesInstaller->getAttribute($entityTypeId, 'fees_shoppingflux')) {
    ...
    try {
        if ($enterpriseOrderGrid) {
            $salesInstaller->run("ALTER TABLE `{$enterpriseOrderGrid}` ADD `fees_shoppingflux` DECIMAL(12, 4) NULL;");
        }
    } catch (Exception $e) {
    }
}
...
if (!$attribute = $salesInstaller->getAttribute($entityTypeId, 'other_shoppingflux')) {
    ...
    try {
        if ($enterpriseOrderGrid) {
            $salesInstaller->run("ALTER TABLE `{$enterpriseOrderGrid}` ADD `other_shoppingflux` VARCHAR(255) NULL;");
        }
    } catch (Exception $e) {
    }
}
...
if (!$attribute = $salesInstaller->getAttribute($entityTypeId, 'shoppingflux_shipment_flag')) {
    ...
    try {
        if ($enterpriseOrderGrid) {
            $salesInstaller->run("ALTER TABLE `{$enterpriseOrderGrid}` ADD `shoppingflux_shipment_flag` INTEGER(11) NULL;");
        }
    } catch (Exception $e) {
    }
}
...
if (!$attribute = $salesInstaller->getAttribute($entityTypeId, 'fees_shoppingflux')) {
    ...
    try {
        if ($enterpriseInvoiceGrid) {
            $salesInstaller->run("ALTER TABLE `{$enterpriseInvoiceGrid}` ADD `fees_shoppingflux` DECIMAL(12, 4) NULL;");
        }
    } catch (Exception $e) {
    }
}
...
if (!$attribute = $salesInstaller->getAttribute($entityTypeId, 'fees_shoppingflux')) {
    ...
    try {
        if ($enterpriseCreditmemoGrid) {
            $salesInstaller->run("ALTER TABLE `{$enterpriseCreditmemoGrid}` ADD `fees_shoppingflux` DECIMAL(12, 4) NULL;");
        }
    } catch (Exception $e) {
    }
}
